...
public:
  ...
  Rate()
      : start_{Time::now()}, actual_cycle_time_{0.0},
        expected_cycle_time_{std::numeric_limits<double>::infinity()} {}

  explicit Rate(const double rate)
      : start_{Time::now()}, actual_cycle_time_{0.0}, expected_cycle_time_{
                                                          1.0 / rate} {}

  explicit Rate(const Duration &d)
      : start_{Time::now()}, actual_cycle_time_{0.0}, expected_cycle_time_{d} {}
  ...
  double frequency() const { return 1.0 / expectedCycleTime().seconds(); }
