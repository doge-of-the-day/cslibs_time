...
public:
  ...
  DurationWindow &operator+=(const duration_t &d) {
    ...
    auto update_Window = [this, n,
                          n_1](const duration_t &d) -> DurationWindow & {
      const duration_t mean_1 = mean_ * n - oldest_;
      const duration_t deviation_1 =
          deviation_ * n - (oldest_ - mean_1) * (oldest_ - mean_);
      mean_ = (mean_1 + d) / n;
  ...
  duration_t const &variance() const { return deviation_; }
